...
##############################################
    ...
    def _orl(self, args):
        if inspect.stack()[1][3] == '_exec':
            print('runnig {} {}'.format(MicroController._orl.__name__, args))

        if not len(args) == 2:
            raise ValueError('incorrect args for _orl')
    ...
    def _xrl(self, args):
        if inspect.stack()[1][3] == '_exec':
            print('runnig {} {}'.format(MicroController._xrl.__name__, args))

        if not len(args) == 2:
            raise ValueError('incorrect args for _xrl')
